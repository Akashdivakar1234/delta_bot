...
class DeltaReversionBot:
    ...
    def check_position_exits(self):
        ...
        if not success:
            log_warning(f"[{self.symbol}] Failed to fetch positions from exchange. Skipping exit check for this cycle.")
            return
        ...
        if self.position_active and pos_match is None:
            ...
            # Fetch latest fills to extract realized PnL and exit price (limit 10 for safety)
